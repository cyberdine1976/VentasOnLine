    ...
    public class CD_Compra
    {
        ...
        public Compra ObtenerDetalleCompra(int IdCompra)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                cmd.CommandType = CommandType.StoredProcedure;
                var NuevaCultura = CultureInfo.GetCultureInfo("en-IN");
                            ...
                            if (doc.Element("DETALLE_COMPRA") != null)
                            {
                                rptDetalleCompra = (from dato in doc.Elements("DETALLE_COMPRA")
                                                    select new Compra()
                                                    {
                                                        ...
                                                        TotalCosto = Convert.ToDecimal(dato.Element("TotalCosto").Value,NuevaCultura),
                                ...
                                rptDetalleCompra.oListaDetalleCompra = (from producto in doc.Element("DETALLE_COMPRA").Element("DETALLE_PRODUCTO").Elements("PRODUCTO")
                                                                        select new DetalleCompra()
                                                                        {
                                                                            ...
                                                                            PrecioUnitarioCompra = Convert.ToDecimal(producto.Element("PrecioUnitarioCompra").Value, NuevaCultura),
                                                                            TotalCosto = Convert.ToDecimal(producto.Element("TotalCosto").Value, NuevaCultura)
                                                                        }).ToList();
                            }
                            else
                            {
                ...
                try
                {
                    ...
                    while (dr.Read())
                    {
                        rptListaCompra.Add(new Compra()
                        {
                            ...
                            TotalCosto = Convert.ToDecimal(dr["TotalCosto"].ToString(), NuevaCultura)
                        });
                    }
                    dr.Close();
        ...
        public List<Compra> ObtenerListaComprasAnuladas(DateTime FechaInicio, DateTime FechaFin, int IdProveedor, int IdTienda)
        {
            List<Compra> rptListaCompra = new List<Compra>();
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                SqlCommand cmd = new SqlCommand("usp_ObtenerListaComprasAnuladas", oConexion);
                cmd.Parameters.AddWithValue("@FechaInicio", FechaInicio);
                cmd.Parameters.AddWithValue("@FechaFin", FechaFin);
                cmd.Parameters.AddWithValue("@IdProveedor", IdProveedor);
                cmd.Parameters.AddWithValue("@IdTienda", IdTienda);
        ...
        public bool AnularCompra(int id)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("usp_AnularCompra", oConexion);
