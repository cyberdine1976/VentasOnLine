...
namespace CapaDatos
{
    public class CD_Venta
    {
        ...
        public bool AnularVenta(int id)
        {
            bool respuesta = true;
                ...
                try
                {
                    ...
                    while (dr.Read())
                    {
                        rptListaVenta.Add(new Venta()
                        {
                            ...
                            oUsuario = new Usuario() { NombreUsuario = dr["Vendedor"].ToString() },
                            TotalCosto = float.Parse(dr["TotalCosto"].ToString()),
                            Activo = Convert.ToBoolean(dr["Activo"]),
                            oTienda = new Tienda() { Nombre = dr["Nombre Tienda"].ToString(),
                            }
        ...
        public List<Venta> ObtenerListaVentasAnuladas(string Codigo, DateTime FechaInicio, DateTime FechaFin, string NumeroDocumento, string Nombre)
        {
            List<Venta> rptListaVenta = new List<Venta>();
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                SqlCommand cmd = new SqlCommand("usp_ObtenerListaVentasAnuladas", oConexion);
                cmd.Parameters.AddWithValue("@Codigo", Codigo);
                cmd.Parameters.AddWithValue("@FechaInicio", FechaInicio);
                cmd.Parameters.AddWithValue("@FechaFin", FechaFin);
                cmd.Parameters.AddWithValue("@NumeroDocumento", NumeroDocumento);
                ...
                try
                {
                    ...
                    while (dr.Read())
                    {
                        rptListaVenta.Add(new Venta()
                        {
                            ...
                            oTienda = new Tienda()
                            {
                                Nombre = dr["Nombre Tienda"].ToString(),
                            }
